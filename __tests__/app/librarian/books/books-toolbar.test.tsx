...
import { BooksToolbar } from '@/app/librarian/books/books-toolbar'
...
describe('BooksToolbar', () => {
  ...
  it('calls router.replace with query after debounce', () => {
    ...
    expect(mockReplace).toHaveBeenCalledWith('/librarian/books?q=dune')
  })

  it('calls router.replace with no query when input is cleared', () => {
    render(<BooksToolbar initialQuery="dune" />)
    fireEvent.click(screen.getByRole('button', { name: /clear/i }))
    expect(mockReplace).toHaveBeenCalledWith('/librarian/books')
  })
